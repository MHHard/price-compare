...
凑单仅从当前店铺页面可见且可验证的商品中选择，优先满足起送差额并减少多余金额。Agent 可以提出候选，但不能凭空编造商品、规格或价格。流程仍停在提交订单前，用户可以在美团最终检查后自行下单。
...
5. 抽象 AutomationObservation 和 RecoveryDecision；
6. 接入失败触发式 AgentRecoveryPlanner；
7. 加入白名单执行器和动作后验证；
8. 持久化 ComparisonSnapshot 并改造结果页和首页卡片；
9. 按单元、录制状态和真实设备三层完成验收。
